 ...
 * <a href=
 * 
 * "http://vase.essex.ac.uk/software/HoughTransform/HoughTransform.java.h tml"
 * 
 * >http://vase.essex.ac.uk/software/HoughTransform/HoughTransform.java.h
 * tml</a>
 * </p>
 * <p/>
	...
	public void addPoints(Gray8Image image) {
		...
		for (int x = 0; x < w; x++) {
			for (int y = 0; y < h; y++) {
				...
				if (((image.getData())[y * w + x]) != Byte.MIN_VALUE) {
					addPoint(x, y);
				}
			}
		}
					...
					for (int dx = -neighbourhoodSize; dx <= neighbourhoodSize; dx++) {
						for (int dy = -neighbourhoodSize; dy <= neighbourhoodSize; dy++) {
							...
							if (houghArray[dt][dr] > peak) {
								// found a bigger pointnearby, skip
								continue loop;
							}
						}
					}
